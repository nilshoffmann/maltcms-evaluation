...
option_list <- list(
    ...
    make_option("--nfiles", default="inputFileNumberCount.csv",
        help="Path relative to dir to table containing number of files used for a tool
        (columns),
...
nfiles <- read.table(opt$nfiles,header=T)
executionMetrics <- read.table(opt$executionMetrics,header=T)
#convert runtime to seconds
#merge by rownames
cases <- merge(parameters,executionMetrics,by="uid",all.x=T)
cases <- merge(cases,nfiles,by="uid",all.x=T)
