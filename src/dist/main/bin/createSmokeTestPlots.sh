...
if [ ! -f "$OUTPUTBASEDIR/$INSTANCENAME/evaluation/evaluation.csv" ]; then
    ...
    groovy/bin/groovy -cp "$MALTCMSHOME:$HSQLCP:$GROOVYCP" "src/main/scripts/CreateTablesSmokeTest.groovy" -c "src/main/scripts/cfg/$INSTANCENAME.groovy" -b . -o "$OUTPUTBASEDIR" -e "$CFGENVIRONMENT"
    retval=$?
    if [ $retval != 0 ] ; then
    ...
    echo "Merging tables"
    #merge tables
    src/main/scripts/R/createEvaluationTableSmokeTest.R --dir="$OUTPUTBASEDIR/$INSTANCENAME/evaluation/"
    retval=$?
    if [ $retval != 0 ] ; then
        echo "Execution failed with value $retval"
        exit $retval
...
src/main/scripts/R/plotResultsSmokeTest.R --dir="$OUTPUTBASEDIR/$INSTANCENAME/evaluation/" --tex=TRUE
retval=$?
if [ $retval != 0 ] ; then
    echo "Execution failed with value $retval"
    exit $retval
